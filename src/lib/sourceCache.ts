    ...
    const inflight = (async () => {
      try {
        ...
        if (entry?.data.length) {
          console.warn(`[AEGIS] ${key} refresh failed — serving ${entry.data.length} cached cameras`);
        ...
        console.warn(`[AEGIS] ${key} fetch failed with no cache to fall back on:`, e);
