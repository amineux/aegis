...
async function geminiOverview(mode: Mode, digest: Digest, keys: string[]): Promise<string | null> {
  try {
    ...
    const model = client.getGenerativeModel({
      ...
      systemInstruction:
        'You are Aegis, a terse intelligence analyst. Given structured facts, write a sharp 2-4 sentence situational read-out. No preamble, no markdown headers, no hedging. Lead with the bottom line.',
    });
    ...
    console.warn('[AEGIS] Gemini overview failed, using heuristic:', e);
    return null;
  }
}
