...
/**
 * Aegis — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'Aegis/1.0', 'Accept': 'application/json' },
    });

    console.log('[AEGIS] IODA response status:', res.status);
    ...
    console.error('[AEGIS] IODA fetch error:', error);
