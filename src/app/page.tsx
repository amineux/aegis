...
export default function Dashboard() {
  ...
  useEffect(() => {
    (window as unknown as { aegisWatchFlight?: (f: WatchedFlight) => void }).aegisWatchFlight = (f) => {
      if (!f?.icao24) return;
  ...
  const [hudTheme, setHudTheme] = useState<'core'|'ghost'>('core');

  useEffect(() => {
    document.body.className = hudTheme === 'core' ? '' : `theme-${hudTheme}`;
  }, [hudTheme]);
  ...
  // ── Drawing / AOI ──
  // AegisMap already owns the draw interaction and the polygon rendering;
  ...
  const handleExportGeoJSON = useCallback(() => {
    downloadFile(
      `aegis-aoi-${new Date().toISOString().slice(0, 10)}.geojson`,
  ...
  useEffect(() => {
    ...
    return loadCameraCatalog(cameras => {
      ...
      setBackendStatus('connected');
    }, () => console.warn('[AEGIS] Camera catalogue load failed; bounded retry scheduled'));
  }, [activeLayers.cctv]);
  ...
  return (
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '64px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          ...
          <div className="flex flex-col items-start gap-0.5">
            <h1 className="text-lg md:text-xl font-bold tracking-[0.28em] text-[#D4AF37] font-mono">{PRODUCT_SHORT}</h1>
        ...
        <div className="flex items-center gap-3 mt-1.5 pl-[44px] min-w-0 pr-4">
          <span className="text-[9px] md:text-[9px] text-[var(--text-muted)] font-mono tracking-[0.2em] md:tracking-[0.3em] uppercase opacity-40 truncate">
            PERSONAL OSINT GRID <span className="hidden md:inline">· FLIGHTS · MARITIME · SATELLITES · CCTV · WEATHER · CYBER THREATS</span>
          </span>
        </div>
      </motion.div>
      ...
      {showLayers && !isMobile && <LayerPanel {...terrainPanelProps} data={data} activeLayers={activeLayers} setActiveLayers={(updater) => { setMissionId(null); setActiveLayers(updater); }} theme={hudTheme} setTheme={setHudTheme} capabilities={capabilities} layerStatus={layerStatus} />}
                ...
                <div className="px-3 pb-3">
                  ...
                  {mobilePanel === 'layers' && (
                    <>
                      ...
                      <LayerPanel {...terrainPanelProps} data={data} activeLayers={activeLayers} setActiveLayers={(updater) => { setMissionId(null); setActiveLayers(updater); }} isMobile={true} theme={hudTheme} setTheme={setHudTheme} capabilities={capabilities} layerStatus={layerStatus} />
                      <div className="mt-8">
